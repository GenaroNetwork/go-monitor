...
type SaveService struct {
	ChanIn chan chan QueryData
}

func NewSaveService(cache uint) SaveService {
	return SaveService{
		ChanIn: make(chan chan QueryData, cache),
	}
}
...
func (s *SaveService) Save(ctx context.Context, queryData QueryData) bool {
	...
	trafficStmt, err = tx.Prepare("insert into users (id) values ($1) on conflict (id) do update set traffic=users.traffic+$2")
	if err != nil {
	...
	bucketStmt, err = tx.Prepare("insert into buckets (id, uid, size, time_start, time_end, backup, name) VALUES ($1, $2, $3, $4, $5, $6, $7) on conflict (id) do nothing")
	if err != nil {
